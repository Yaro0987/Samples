...
/* ==========================================
   ECOMMERCE HEADER - Sticky + Mobile Nav
   ========================================== */
(function() {
  /* Sticky header */
  var mainHeader = document.getElementById('ec-sticky-header');
  var navBar = mainHeader ? mainHeader.closest('.ec-nav-bar') || document.querySelector('.ec-nav-bar') : null;
  var headerHeight = mainHeader ? mainHeader.offsetHeight : 0;

  function handleSticky() {
    if (!mainHeader) return;
    var scrollY = window.pageYOffset || document.documentElement.scrollTop;
    var navBarEl = document.querySelector('.ec-nav-bar');
    if (scrollY > 150) {
      mainHeader.classList.add('ec-scrolled');
      if (navBarEl) navBarEl.style.top = mainHeader.offsetHeight + 'px';
      document.body.style.paddingTop = (mainHeader.offsetHeight + (navBarEl ? navBarEl.offsetHeight : 0)) + 'px';
    } else {
      mainHeader.classList.remove('ec-scrolled');
  ...
  /* Mobile drawer */
  var hamburger = document.getElementById('ec-hamburger');
  var mobileNav = document.getElementById('ec-mobile-nav');
  var mobileOverlay = document.getElementById('ec-mobile-overlay');
  var mobileClose = document.getElementById('ec-mobile-nav-close');
  ...
  /* Accordion menu in mobile nav */
  var mobileMenuItems = document.querySelectorAll('.ec-mobile-nav-body .menu-item-has-children > a');
  mobileMenuItems.forEach(function(item) {
    item.addEventListener('click', function(e) {
      ...
      siblings.forEach(function(sib) { sib.classList.remove('menu-open'); });
  ...
  var navLinks = document.querySelectorAll('.ec-main-nav a, .ec-mobile-nav-body a');
  navLinks.forEach(function(link) {
    var href = link.getAttribute('href');
    if (href === currentPage) {
      link.parentElement.classList.add('active');
      /* Also open parent accordion in mobile nav */
      var parentMenu = link.closest('.sub-menu');
      if (parentMenu) {
        parentMenu.parentElement.classList.add('menu-open');
      }
    }
  });
})();
...
function updateCartTotals() {
  var rows = document.querySelectorAll('.cart-table:not(.head) li');
  rows.forEach(function(row) {
    var input = row.querySelector('.quantity-control input');
    var totalEl = row.querySelector('.c-total span');
    var priceEl = row.querySelector('.c-price .orgnl');
...
/* Hero slider - change background per slide */
(function() {
  var slider = document.querySelector('.f-slider-one');
  var section = slider ? slider.closest('[data-slides-bg]') : null;
  if (!slider || !section) return;
  
  var bgStr = section.getAttribute('data-slides-bg');
  if (!bgStr) return;
  var bgs = bgStr.split(',');
  ...
  function onSlideChanged() {
    var items = slider.querySelectorAll('.owl-item.active');
    if (items.length > 0) {
      // Get the index of the first active item
      var allItems = slider.querySelectorAll('.owl-item');
      for (var i = 0; i < allItems.length; i++) {
        if (allItems[i].classList.contains('active')) {
          var idx = i % bgs.length;
          section.style.backgroundImage = 'url(' + bgs[idx] + ')';
          break;
        }
      }
    }
  ...
  if (window.jQuery) {
    jQuery(slider).on('changed.owl.carousel', function(e) {
      var idx = e.item.index - e.relatedTarget._clones.length;
      var realIdx = idx % bgs.length;
      if (realIdx < 0) realIdx += bgs.length;
      section.style.backgroundImage = 'url(' + bgs[realIdx] + ')';
    });
  }
})();